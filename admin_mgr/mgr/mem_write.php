	...
	<td>
		...
		<tr>
			...
			<td valign="top" style="padding:10px 15px;">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
				  <tr>
					<td colspan="2" align="left" class="title"><img src="../images/icon/bullet.gif" align="absmiddle" />관리자관리 > 상세보기</td>
				  </tr>
				</table>
				<form name="actfm" id="actfm"method="post" action="mem_proc.php" target="_proc">
					<input type="hidden" name="cmd" id="cmd" value="<?= $cmd ?>">
					<input type="hidden" name="pre_cmd" id="pre_cmd" value="all">
					...
					<tr style="display: <? if(!eqyn(get_admin_auth(), "A")){ echo "none"; }?>">
				...
				<tr>
					...
					if(eqyn($am_userid, "")){
					?>
						<button type="button" class="btn btn02" onclick="regi('ins');"> 등록 </button>
					<?
					}else{
					?>
						<button type="button" class="btn btn02" onclick="regi('upt');"> 수정 </button>
						<button type="button" class="btn btn05" onclick="regi('del');"> 삭제 </button>				
					<?
					}
					?>
						<button type="button" class="btn btn03" onclick="location.href='mem_list.php?<?= $GLB_RETURN_PARAM ?>'"> 리스트 </button>
					</td>
				</tr>
				</table>
			</td>
